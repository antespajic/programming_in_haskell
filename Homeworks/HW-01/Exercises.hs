...
showSalary amount bonus
  | amount > -1 = "Salary is " ++ show amount ++ (if bonus /= 0 then ", and a bonus " ++ show bonus else "") 
  | otherwise = "Salary can't be negative"
...
-- last three elements.
ex211 = dropFirstLast3
dropFirstLast3 :: String -> String
dropFirstLast3 x = drop 3 $ take (length x - 3) x
...
concatLongerFirst :: String -> String -> String
concatLongerFirst s1 s2  
...
ex221 = doublesFromTo
doublesFromTo :: (Num a, Ord a, Enum a) => a -> a -> [a]
doublesFromTo a b 
  | a < b = [x*2 | x <- [a..b]]
  | otherwise = [x*2 | x <- [b..a]]
...
caesarCode n xs = [
    ...
    else b | b <- [toLower a | a <- concat $ words xs]
  ]
...
toLowerCaseWithoutSpaces :: String -> String
toLowerCaseWithoutSpaces s = [ toLower c | c <- s, c /= ' ']
ex232 = isPalindrome
isPalindrome :: String -> Bool
isPalindrome s = (reverse $ toLowerCaseWithoutSpaces s) == toLowerCaseWithoutSpaces s
...
-- additional argument.
ex241 = inCircle
inCircle :: Int -> Int -> Int -> Int -> Int -> [(Int, Int)]
inCircle r x y resX resY = [(cx, cy) | cx <- [0..resX], cy <- [0..resY], (cx - x)^2 - (cy - y)^2 < r^2]
...
indices x xs = [ fst c | c <- zip [0..] xs, (snd c) == x]
...
showLineNumbers s = concat [(show $ fst c) ++ " " ++ snd c ++ "\n" | c <- zip [1..] $ lines s]
...
haveAlignment xs ys = or [ fst pair == snd pair | pair <- zip xs ys ]
...
ex254 = common
common :: Eq a => [a] -> [a] -> [a]
common xs ys = [ fst p | p <- zip xs ys, fst p == snd p ]
